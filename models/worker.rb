class Worker
  attr_accessor :first_name, :last_name, :courses_taught, :education, :image, :title, :job, :position
  ...
  def self.pull(worker_type)
    return [] unless %w(faculty staff volunteers advisory board ).include?(worker_type)
